...
#                                          relevance_score=1.0, # 系统职能类始终保留
#                                          metadata={"type":"system_instruaction", "priority": "high"}))
...
#                 if memory_results and "未找到" not in memory_results:
#                     packets.append(ContextPacket(
#                         content=memory_results,
#                         metadata={"type": "related_memory"}
#                     ))
#             except Exception as e:
#                 logger.warning(f"记忆 检索失败: {e} memory_tool try .. failed !!!");
...
#         logger.info(f"[ContextBuilder] 汇集了 {len(packets)} 个候选信息包")
#         return packets
...
#         remaining_tokens = available_token - system_tokens;

#         if remaining_tokens<= 0:
#             logger.warning(f"系统指令已占满所有 token 预算")
...
#             # 计算相关性分数 （如果尚未计算）
#             if packet.relevance_score == 0.5: # 默认值， 需要从新计算
...
def count_tokens(text: str) -> int:
    ...
    try:
        ...
        # 降级方案：粗略估算（1 token ≈ 4 字符）
        logger.warning("⚠️ not use tiktoken , use char");
        return len(text) // 4
