...
def create_gradio_ui():
    ...
    def init_assistant(user_id: str) -> str:
        ...
        if not user_id:
            user_id = "web_user"
        assistant_state["assistant"] = LearningAssistant(user_id=user_id)
    ...
    with gr.Blocks(title="智能文档问答助手", theme=gr.themes.Soft()) as demo:
        ...
        基于Agents的智能文档问答系统，支持：
        - 📄 加载PDF文档或者Makedown文档并构建知识库
        - 💬 智能问答（基于RAG）
        - 📝 学习笔记记录
        - 🧠 学习历程回顾
        - 📊 学习报告生成
        ...
        with gr.Tab("🏠 开始使用"):
            ...
            gr.Markdown("### 📄 加载PDF文档或Makedown文档")
            pdf_upload = gr.File(
                label="上传PDF文件或Makedown文件",
                file_types=[".pdf", ".md"],
                type="filepath",
                #file_count="multiple", # 多个文件上转
            )
