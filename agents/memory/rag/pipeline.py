...
#         logger.info(f"[WARNING] MarkItDown failed for {path}: {e}")
#         return _fallback_text_reader(path)
...
#     except Exception as e:
#         logger.info(f"[WARNING] Enhanced PDF processing failed for {path}: {e}")
#         return _fallback_text_reader(path)
...
#             logger.info(f"[WARNING] No content extracted from: {path}")
#             continue
...
# def normalize_vectors(vecs, dimension):
#     # out = []
...
#     #     # 排除 str
#     #     if isinstance(v, str):
#     #         raise TypeError(f"embedder returned a string: {v}")
...
#     #         raise TypeError(f"Invalid vector type: {type(v)}")

#     #     # 转 float
#     #     try:
#     #         v = [float(x) for x in v]
#     #     except Exception as e:
#     #         raise ValueError(f"Vector contains non-numeric values: {e}")

#     #     # 补零或截断到 dimension
#     #     if len(v) < dimension:
#     #         v.extend([0.0] * (dimension - len(v)))
...
#     # return out
#     """
#     将 embedder 输出统一成 List[List[float]]
#     """
#     out = []

#     # 如果单个向量不是 list/tuple/np.ndarray，就包装成 list
#     if not isinstance(vecs, list):
#         vecs = [vecs]
...
#         # 先排除 str
#         if isinstance(v, str):
#             # 如果 embedder 返回了 CSV 字符串或错误字符串，尝试用 0 填充
#             out.append([0.0] * dimension)
#             continue
...
#             # 获取向量
#             logger.info(f"Part type: {type(part)}, content preview: {part}")
#             part_vecs = embedder.encode(part)
#             logger.info(f"Embedder output type: {type(part_vecs)}, preview: {part_vecs}")
...
#                         logger.info(f"[WARNING] 向量维度异常: 期望{dimension}, 实际{len(v_norm)}")
#                         # 用零向量填充或截断
#                         if len(v_norm) < dimension:
...
#                     vecs.append(v_norm)
#                 except Exception as e:
#                     logger.info(f"[WARNING] 向量转换失败: {e}, 使用零向量")
#                     vecs.append([0.0] * dimension)
#             # 归一化成 List[List[float]]
#             #part_vecs = normalize_vectors(part_vecs, dimension)
#             # 直接添加到 vecs
#           #  vecs.extend(part_vecs)  # 直接追加，已经保证类型安全
#             # for v in part_vecs:
#             #     try:
#             #         vecs.append(v)
#             #     except Exception as e:
#             #         logger.info(f"[WARNING] 单个向量异常: {e}, 使用零向量")
#             #         vecs.append([0.0] * dimension)  
#         except Exception as e:
#             logger.info(f"[WARNING] Batch {i} encoding failed: {e}")
...
#                             logger.info(f"[WARNING] 小批次向量转换失败: {e2}")
#                             vecs.append([0.0] * dimension)
#                 except Exception as e2:
#                     logger.info(f"[WARNING] 小批次 {j//8} 仍然失败: {e2}")
#                     # 为这个小批次创建零向量
#                     for _ in range(len(small_part)):
#                         vecs.append([0.0] * dimension)
            
#             if not success:
#                 logger.info(f"[ERROR] 批次 {i} 完全失败，使用零向量")
...
#     logger.info(f"[RAG] Qdrant upsert start: n={len(vecs)}")
#     success = store.add_vectors(vectors=vecs, metadata=metas, ids=ids)
#     if success:
#         logger.info(f"[RAG] Qdrant upsert done: {len(vecs)} vectors indexed")
#     else:
#         logger.info(f"[RAG] Qdrant upsert failed")
...
#             logger.info(f"[WARNING] Query向量维度异常: 期望{dimension}, 实际{len(result)}")
#             # 用零向量填充或截断
#             if len(result) < dimension:
...
#         return result
#     except Exception as e:
#         logger.info(f"[WARNING] Query embedding failed: {e}")
...
#         )
#     except Exception as e:
#         logger.info(f"[WARNING] RAG search failed: {e}")
#         return []
