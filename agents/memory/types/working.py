...
#         super().__init__(config=config, storage_backend=storage_backend)
#         # 工作记忆特定配置
...
#         self.max_age_minutes = getattr(self.config, 'working_memory_ttl_minutes', 120) #self.config.working_memory_ttl_minutes or 60;
...
#     """混合检索：TF-IDF向量化 + 关键词匹配"""
#     def retrieve(self, query, limit = 5, **kwargs) ->List[MemoryItem]:
#        self._expire_old_memories()

#        # 尝试TF-IDF向量检索
#        vector_scores = self._try_tfidf_search(self);
...
#             heapq.headpush(self.memory_heap, (-priority, mem.timestamp, mem));
